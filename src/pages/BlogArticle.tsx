...
function renderContent(content: string) {
  ...
  return blocks.map((block, i) => {
    ...
    if (trimmed.startsWith("## ")) {
      return (
        <h2 key={i} className="font-display text-2xl font-bold uppercase tracking-tight mt-10 mb-4 text-foreground">
          {trimmed.slice(3)}
        </h2>
      );
    }
    ...
    if (trimmed.startsWith("> ")) {
      return (
        <blockquote
          key={i}
          className="border-l-2 border-primary pl-5 my-8 text-primary/90 italic text-lg leading-relaxed"
        >
          {trimmed.slice(2)}
        </blockquote>
      );
    ...
    return (
      <p
        key={i}
        className="text-muted-foreground leading-[1.8] mb-5"
...
export default function BlogArticle() {
  ...
  const jsonLd = {
    ...
    author: { "@type": "Organization", name: "NEXOV" },
    publisher: { "@type": "Organization", name: "NEXOV" },
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-28 pb-20">
        <article className="container max-w-[720px]">
          ...
          <h1 className="font-display text-3xl md:text-4xl font-bold uppercase mt-2 tracking-tight leading-tight">
            {article.title}
          </h1>
          ...
          <div className="mt-14 p-8 rounded-2xl bg-card border border-primary/30">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-5">
              ...
              <Button
                variant="outline"
                size="lg"
                className="font-semibold gap-2 border-primary/30 text-primary hover:bg-primary/10"
                asChild
              >
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {related.map((r) => (
                  <Link
                    key={r.slug}
                    to={`/blog/${r.slug}`}
                    className="group block bg-card border border-border rounded-xl p-5 transition-all duration-300 hover:border-primary/50 hover:-translate-y-1"
                  >
                    <span className="font-mono text-[10px] text-primary uppercase tracking-widest">
                      {r.category}
                    </span>
                    <h4 className="font-display text-sm font-bold uppercase mt-1.5 tracking-tight group-hover:text-primary transition-colors leading-tight">
                      {r.title}
                    </h4>
